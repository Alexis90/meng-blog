...
def register():
    ...
    if request.method == 'POST' and register_form.validate_on_submit():
        ...
        if db.session.execute(db.select(User).filter_by(email=email)).scalar():
            ...
            password_hashed = generate_password_hash(request.form.get('password'), method='pbkdf2:sha256',
                                                     salt_length=8)
...
def login():
    ...
    if request.method == 'POST' and login_form.validate_on_submit():
        ...
        if not user:
            flash('User doesn\'t exist, please sign up first.')
...
def show_post(post_id):
    ...
    if request.method == 'POST' and comment_form.validate_on_submit():
        if current_user.is_authenticated:
            new_comment = Comment(body=request.form.get('body'), author=current_user, post=requested_post)
